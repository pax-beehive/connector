...
import type { Tenant, ViewId } from "../domain/console";
...
export function Sidebar({ active, open, onSelect, onClose }: { active: ViewId; open: boolean; onSelect: (view: ViewId) => void; onClose: () => void }) {
  return (
    <aside className={`sidebar ${open ? "sidebar-open" : ""}`}>
      ...
      <div className="environment-card">
        ...
        <div><strong>Prototype</strong><small>No live backend</small></div>
      </div>
      ...
      <div className="operator-card">
        ...
        <div><strong>Operator session</strong><small>Identity adapter pending</small></div>
      </div>
    </aside>
  );
}
...
export function Topbar({ theme, tenants, tenantId, onTenantChange, onThemeChange, onMenu }: { theme: "dark" | "light"; tenants: Tenant[]; tenantId: string; onTenantChange: (tenantId: string) => void; onThemeChange: () => void; onMenu: () => void }) {
  return (
