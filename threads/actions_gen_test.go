...
func TestGeneratedActionDispatchErrors(t *testing.T) {
	...
	_, err = c.InvokeAction(ctx, connector.ActionCall{Provider: "threads", Method: "Missing"})
	...
	mu.Unlock()
	_, err = c.InvokeAction(ctx, connector.ActionCall{Provider: "threads", Method: first.name})
